...
def api_verify_onchain(token: str):
    ...
    try:
        ...
        # Record real transaction in audit log
        add_audit_entry(
            action=f"{token} — TEE Verification (on-chain)",
...
def add_audit_entry(action: str, tx_hash: str, payment_hash: str,
                    model: str, settlement: str, tee_signature: str = None):
    """Add a real transaction to the audit log."""
    # Determine best hash to display
    display_hash = None
    explorer_url = None
    if tx_hash and tx_hash != "external":
        display_hash = tx_hash
        explorer_url = f"https://sepolia.basescan.org/tx/{tx_hash}"
    elif payment_hash and payment_hash != "external" and payment_hash:
        display_hash = payment_hash
        explorer_url = f"https://sepolia.basescan.org/tx/{payment_hash}"
    else:
        display_hash = "batch-settlement"
        explorer_url = None
    ...
    entry = {
        ...
        "tee_signature": tee_signature[:20] + "..." if tee_signature and len(tee_signature) > 20 else tee_signature,
        "status": "verified"
    }
    with _audit_lock:
        _audit_log.insert(0, entry)
        if len(_audit_log) > 50:  # keep last 50 entries
            _audit_log.pop()
